 @if(!empty($record))
 <form class="form" action="{{ url('admin/cities/edit/'.$record->id) }}" method="post" enctype="multipart/form-data">
 @else
                        ...
                        <div class="row">
                           ...
                           <div class="col-xl-7 my-2">
                             <!--begin::Group-->
                              ...
                              <!--end::Group-->
                                <div class="form-group row">
                                 <label class="col-form-label col-3 text-lg-right text-left">  {{ __('manage.State Name') }}</label>
                                 <div class="col-9">
                                 ...
                                 @endphp
                                    ...
                                    <select class="form-control form-control-lg form-control-solid" name="state_id">
                                     <option value=""> {{ __('manage.Select') }}</option>
                                    @foreach($get_state as $value_s)
                                      <option {{ ($state_id == $value_s->id) ? 'selected' : '' }} value="{{ $value_s->id }}">{{ $value_s->getName() }}</option>
                                    @endforeach
                                    </select>
                                    <div style="color: red;">{{ $errors->first('state_id') }}</div>
                                 </div>
                              </div>
                              ...
                              <div class="col-xl-7">
                                 <div class="row">
                                    ...
                                    <div class="col-9">
                                       ...
                                       <a href="{{ url('admin/cities') }}" class="btn btn-clean font-weight-bold">{{ __('manage.Cancel') }}</a>
                                    </div>
                                 </div>
                              </div>
                           </div>
